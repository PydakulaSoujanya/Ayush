...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = "INSERT INTO expenses_claim 
              (employee_name, expense_category, expense_date, amount_claimed, attachment, status, rejection_reason, submitted_date, approved_date, payment_status, payment_date, description) 
              VALUES 
              ('$employee_name', '$expense_category', '$expense_date', '$amount_claimed', '$attachment', '$status', '$rejection_reason', '$submitted_date', '$approved_date', '$payment_status', '$payment_date', '$description')";

    if (mysqli_query($conn, $query)) {
        echo "<script>alert('Expense claim submitted successfully!'); window.location.href='expenses_claim_form.php';</script>";
    } else {
